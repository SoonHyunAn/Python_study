...
# logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(levelname)s] %(message)s") #시간, 로그정보, 메시지
# # level=logging 에 Info를 쓰면 Info 이상, Debug면 Debug 이상 메시지 출력
...
# 로그 레벨 설정
logger.setLevel(logging.INFO) #대문자로 레벨 설정해야함
